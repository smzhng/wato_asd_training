...
void CostmapNode::laserCallback(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
    ...
    int range_size = static_cast<int>(std::cell((2.0*range_max)/resolution))+1; //static cast converts it to int basically
    ...
    for (int i = 0; i<count; i++){ //goes through the full range of beams
        ...
        int x = static_cast<int>((ranges*std::sin(angle))/resolution)+origin;
        int y = static_cast<int>((ranges*std::cos(angle))/resolution)+origin;
